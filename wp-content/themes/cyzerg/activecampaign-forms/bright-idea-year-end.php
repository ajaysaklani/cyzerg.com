<form id="_form_1109" accept-charset="utf-8" action="//cyzerg.activehosted.com/proc.php" enctype="multipart/form-data" method="post" _lpchecked="1">
    <h3><?php echo $h3; ?></h3>
<div style="display:none;">
    <input type="hidden" name="f" value="1109"><p></p>
...
<div class="form-inner" style="padding:0;"><input type="text" name="fullname" placeholder="Full Name:" size="40" required aria-required=”true” style="margin-left:-10px;"><br>
    ...
    <input type="text" name="phone" placeholder="Phone Number:" size="40" required aria-required=”true” pattern="(\+\d*)?[\( ]?\d{3}[-\) ]?\d{3}[- ]?\d{4}"  title="format: 555-555-1212" style="margin-left:-10px;"><br>
        <textarea class="wpcf7-textarea" cols="40" name="field[8]" placeholder="Any specific concerns for 2015?" rows="6" style="margin-left:-10px;height:110px;"></textarea><br>
        <input type="submit" value="Let's Meet" name="submit" class="wpcf7-submit" style="margin-left:-10px;padding: 10px; background:#229445;box-sizing:content-box;width:100%;max-width: 377px;">
</div>
</form>
<script>
    (function($) {
            $(function(){
                if($(window).width()>980) {
                    $('#_form_1109').closest('.et_pb_column_1_3').hcSticky({innerTop:-90});
                }
            });
    })(jQuery);
</script>
