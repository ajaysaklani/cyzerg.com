...
function easy_testimonials_setup_js() {
	...
	if(!$disable_cycle2){
		wp_enqueue_script(
			'ezt_cycle2',
			plugins_url('include/js/jquery.cycle2.js', __FILE__),
			array( 'jquery' ),
			false,
			true
		); 
wp_enqueue_script(
			'ezt_cycle2',
			plugins_url('include/js/jquery.cycle2.swipe.min.js', __FILE__),
			array( 'jquery' ),
			false,
			true
		);		
		wp_enqueue_script(
			'ezt_cycle2',
			plugins_url('include/js/ios6fix.js', __FILE__),
			array( 'jquery' ),
			false,
			true
		);	
...
function easy_t_get_the_excerpt( $post_id )
{
  ...
  $output = easy_t_trim_excerpt( '' , $post);
...
function easy_t_excerpt_more( $more ) {
	...
	if(get_option('easy_t_link_excerpt_to_full', false)){
		return ' <a class="more-link" href="' . get_permalink( $post->ID ) . '">' . get_option('easy_t_excerpt_text') . '</a>';
	} else {
