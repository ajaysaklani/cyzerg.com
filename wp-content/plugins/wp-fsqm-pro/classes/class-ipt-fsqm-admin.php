...
class IPT_FSQM_Form_Table extends WP_List_Table {
	...
	public function prepare_items() {
		...
		if ( ! empty( $_GET['s'] ) ) {
			$search = '%' . $_GET['s'] . '%';
			$wheres[] = $wpdb->prepare( "name LIKE %s", $search );
		}
...
class IPT_FSQM_Data_Table extends WP_List_Table {
	...
	public function prepare_items() {
		...
		//prepare our query
		$query = "SELECT d.id id, d.f_name f_name, d.l_name l_name, d.email email, d.phone phone, d.ip ip, d.date date, d.star star, d.score score, d.max_score max_score, d.user_id user_id, f.name name, f.id form_id, c.name catname FROM {$ipt_fsqm_info['data_table']} d LEFT JOIN {$ipt_fsqm_info['form_table']} f ON d.form_id = f.id LEFT JOIN {$ipt_fsqm_info['category_table']} c ON f.category = c.id";
