	...
	var methods = {
		...
		applyDataTable : function(op) {
			...
			$(this).find('.ipt_fsqm_up_table').iptPluginUIFFront({
				callback : function() {
					var dt = $(_self).find('.ipt_fsqm_up_table').show().dataTable({
						"bJQueryUI": true,
					...
					if ( op.settings.filters == '1' ) {
						var columnFilters = [];
						columnFilters[columnFilters.length] = { column_number: columnFilters.length, column_data_type: 'html', html_data_type: 'text', filter_default_label: iptFSQMUP.l10n.filters.form, filter_match_mode: 'exact' };
						if ( op.settings.showcategory == '1' ) {
							columnFilters[columnFilters.length] = { column_number: columnFilters.length, column_data_type: 'html', html_data_type: 'text', filter_default_label: iptFSQMUP.l10n.filters.category, filter_match_mode: 'exact' };
						}

						dt.yadcf(columnFilters, 'header');
						$('.yadcf-filter-wrapper').find('select').addClass('ipt_uif_select');
						$('.yadcf-filter-wrapper').find('input.yadcf-filter-range-date').addClass('ipt_uif_text').attr('type', 'text');
					}
