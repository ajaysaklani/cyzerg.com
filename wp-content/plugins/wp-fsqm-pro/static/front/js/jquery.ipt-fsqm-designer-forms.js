		...
		updateProgressBar: function( newTab ) {
			...
			for ( var i = 0; i < this.steps; i++ ) {
				...
				if ( this.tabIndices.eq(i).hasClass('iptUIFCHidden') ) {
					this.newPB.find('.pb_circleouter_' + i).addClass('pb_hidden').next('.ipt_fsqm_designer_pb_circleinner').find('.ipticm').removeClass('ipt-icomoon-checkmark').addClass('ipt-icomoon-close');
				} else {
					this.newPB.find('.pb_circleouter_' + i).removeClass('pb_hidden').next('.ipt_fsqm_designer_pb_circleinner').find('.ipticm').removeClass('ipt-icomoon-close').addClass('ipt-icomoon-checkmark');
				}
			}
